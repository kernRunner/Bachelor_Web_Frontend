        ...
        <div class="pt-16 md:pt-32 lg:pt-44 pb-16 max-w-6xl mx-10 my-auto w-full">
            ...
            <div class="mt-20 grid grid-cols-3">
                <div class="sticky top-[150px] h-fit">
                    ...
                    <div class="flex items-center">
                        ...
                        <p class="text-xl text-gray mb-1 ml-4 cursor-pointer" @click="jumpToId('Attractions')">Attractions
                        </p>
                    </div>
                    <div class="border-l border-black/10 ml-3">
                        <div v-for="(item, index) in content" class="flex items-center my-3">
                            ...
                            <p class="ml-8 cursor-pointer" @click="jumpToId(index)"
                                :class="[currentSection == index ? 'text-secondaryColor font-bold' : 'text-gray']">{{
                                    item.Name }}
                            </p>
                        </div>
                    </div>
                    <div class="flex items-center mt-3">
                        ...
                        <p class="text-xl text-gray mb-1 ml-4" @click="jumpToId('Discover')">Discover</p>
                    </div>
                    <!-- <div class="flex items-center mt-3">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                            <path
                ...
                <div class="col-span-2 ml-3">
                    ...
                    <div v-for="(item, index) in content" class="content scroll-mt-24" :id="index">
                        <div class="slideText">
                            <p class="font-bold text-2xl mt-12">{{ item.Name }}</p>
                            <p class="mt-6 text-sm">{{ item.Description }}</p>
                        </div>
                        <div class="relative w-3/4 contentSlot">
                            <div class="absolute bottom-0 right-0 p-3 bg-black/50 rounded-tl-[19px] rounded-br-[19px] cursor-pointer"
                                @click="counter.toggelFavorite(item.id)">
                                ...
                                <div :class="counter.favorite.includes(item.id) ? 'heart' : 'opacity-0'"
                                    :style="{ 'background-position': counter.favorite.includes(item.id) ? '-2800px 0' : '0 0' }">
                                </div>
                            </div>
                            <img  :src="'/src/assets/img/' + country + '/Region/' + item.Key + '/' + item.Img" :alt="item.Ref" class="rounded-[20px] mt-6">
                            <!-- <img  v-if="!val" src="@/assets/img/Global/defaultImage.jpg" alt="" class="rounded-[20px] mt-6"> -->
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- <div class="slideshow-container">
                <img v-for="(image, index) in images" :key="index" :src="image.src"
                    :class="{ 'mySlides': true, 'active': index === currentIndex1 }">
            </div>
            <div id="default-carousel" class="relative w-full" data-carousel="slide">

                <div class="relative h-56 overflow-hidden rounded-lg md:h-96">
                    <transition name="fade" mode="out-in">
                        <img :key="currentIndex" :src="carouselItems[currentIndex].src"
                ...
                <div class="absolute z-30 flex space-x-3 -translate-x-1/2 bottom-5 left-1/2">
                    <button v-for="(item, index) in carouselItems" :key="index" type="button" class="w-3 h-3 rounded-full"
                        :class="[currentIndex == index ? ' bg-secondaryColor' : 'bg-white']"
                        :aria-current="currentIndex === index" :aria-label="`Slide ${index + 1}`" @click="goToSlide(index)"
